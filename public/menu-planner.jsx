...
export default function App() {
  ...
  const [babyAge, setBabyAge] = useState(() => getStorage("babyAge", 9));
  ...
  useEffect(() => { setStorage("babyAge", babyAge); }, [babyAge]);
  ...
  // Retourne l'adaptation bébé adaptée à l'âge (depuis le champ statique ou générée)
  function getBabyAdaptation(dish) {
    if (babyAge < 6) return "Allaitement / lait infantile uniquement — pas de solides avant 6 mois";
    if (babyAge < 8) return dish.baby6 || dish.baby || "Purée lisse, texture très fine, sans sel ni sucre";
    if (babyAge < 12) return dish.baby || "Purée ou morceaux très mous, sans sel";
    if (babyAge < 18) return dish.baby12 || dish.baby || "Morceaux mous, version peu salée";
    return dish.baby18 || dish.baby || "Version enfant : même plat, moins épicé, morceaux adaptés";
  }

  function babyAgeLabel(months) {
  ...
  if (step === "accueil") return (
    <div style={S.app}>
      ...
      <div style={S.content}>
        ...
        <div style={{ background: "#fff", borderRadius: 18, padding: 16, marginBottom: 14, border: "1.5px solid #e8ebe8" }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#1a2e1c", marginBottom: 12 }}>👶 Âge de bébé</div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <input type="range" min={4} max={36} value={babyAge} onChange={e => setBabyAge(Number(e.target.value))}
              style={{ flex: 1, accentColor: "#2d5a3d", height: 4, cursor: "pointer" }} />
            <div style={{ background: "#e8f0e9", borderRadius: 20, padding: "4px 14px", fontSize: 14, fontWeight: 700, color: "#2d5a3d", minWidth: 80, textAlign: "center", flexShrink: 0 }}>
              {babyAgeLabel(babyAge)}
            </div>
          </div>
          <div style={{ fontSize: 11, color: "#aaa", marginTop: 8 }}>
            {babyAge < 6 ? "⚠️ Avant 6 mois : lait uniquement" : babyAge < 12 ? "Diversification alimentaire" : babyAge < 18 ? "Alimentation variée, morceaux mous" : "Repas en morceaux adaptés"}
          </div>
        </div>
